...
import { StyleSheet, View, Text, TouchableOpacity, Image, Alert, ScrollView, ActivityIndicator, Platform, ToastAndroid } from 'react-native';
...
const VoucherDetails = ({ navigation, route }) => {
  ...
  const { fuelVoucherId } = route.params;
  ...
  useEffect(() => {
    const fetchReceipt = async () => {
      try {
        ...
        console.log("getFeulVoucherById ID:", response);
    ...
    const fetchAttachmentById = async () => {
      try {
        ...
        if (response.status === 1 && response.data?.length > 0) {
          setattachmentUri(response.data[0]);
        } else {
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.receiptBox}>
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false}>
            <Text style={[styles.value, { flexWrap: 'nowrap' }]}>
        ...
        {attachmentUri ? (
          <Image
            source={{ uri: BASE_API_URL + attachmentUri?.attachment }}
